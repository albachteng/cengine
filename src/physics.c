...
void physics_world_init(PhysicsWorld* world, ECS* ecs, ComponentType transform_type) {
    ...
    ComponentMask physics_mask = (1ULL << world->transform_type) | (1ULL << world->verlet_type);
...
Entity physics_create_circle(PhysicsWorld* world, Vec3 position, float radius, float mass) {
    ...
    Transform* transform = (Transform*)ecs_add_component(world->ecs, entity, world->transform_type);
    *transform = transform_create(position.x, position.y, position.z);
    
    VerletBody* verlet = (VerletBody*)ecs_add_component(world->ecs, entity, world->verlet_type);
    verlet->velocity = vec3_zero();
    verlet->acceleration = vec3_zero();
    verlet->old_position = position;
    
    CircleCollider* collider = (CircleCollider*)ecs_add_component(world->ecs, entity, world->collider_type);
...
void physics_verlet_integration(PhysicsWorld* world, float delta_time) {
    for (Entity entity = 1; entity < world->ecs->next_entity_id; entity++) {
        ...
        Vec3 new_position = vec3_add(current_position, 
            vec3_add(vec3_multiply(vec3_add(current_position, vec3_multiply(verlet->old_position, -1.0f)), world->damping),
...
void physics_solve_collisions(PhysicsWorld* world) {
    for (Entity entity1 = 1; entity1 < world->ecs->next_entity_id; entity1++) {
        if (!ecs_entity_active(world->ecs, entity1)) continue;
        ...
        for (Entity entity2 = entity1 + 1; entity2 < world->ecs->next_entity_id; entity2++) {
            if (!ecs_entity_active(world->ecs, entity2)) continue;
            ...
            Vec3 normal;
            float penetration;
            if (circle_circle_collision(t1->position, c1->radius, t2->position, c2->radius, &normal, &penetration)) {
